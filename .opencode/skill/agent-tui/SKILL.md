...
When driving **amux** with agent-tui, set these environment variables to avoid
timing and key-mapping issues:

```bash
# Remap the prefix key to something reliable over PTY (default: ctrl+space)
export AMUX_PREFIX_KEY=ctrl+p

# Extend the prefix timeout so the palette doesn't vanish before you can act
export AMUX_PREFIX_TIMEOUT=30s
```

Both variables are read at startup. Pass them on the `agent-tui run` line:

```bash
agent-tui run --env AMUX_PREFIX_KEY=ctrl+p --env AMUX_PREFIX_TIMEOUT=30s -- amux
```
