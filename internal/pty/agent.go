...
type agentCmd struct {
	...
	// inject (test-only): pre-built agents to add
	injectAgents []*Agent
	injectWsID   data.WorkspaceID
}
...
func (m *AgentManager) handle(cmd *agentCmd, agents map[data.WorkspaceID][]*Agent, tmuxOpts *tmux.Options) {
	...
	case "create_agent", "create_viewer":
	...
	case "inject_agents":
		agents[cmd.injectWsID] = append(agents[cmd.injectWsID], cmd.injectAgents...)
		cmd.respCh <- agentResp{}
	}
}
...
// injectAgentsForTest sends pre-built agents through the command channel for
// testing purposes. The agents are added to the internal map without creating
// real tmux sessions.
func (m *AgentManager) injectAgentsForTest(wsID data.WorkspaceID, agents []*Agent) {
	respCh := make(chan agentResp, 1)
	m.cmdCh <- agentCmd{kind: "inject_agents", injectWsID: wsID, injectAgents: agents, respCh: respCh}
	<-respCh
}
