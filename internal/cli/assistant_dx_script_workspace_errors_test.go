...
func TestAssistantDXWorkspaceCreate_RejectsProjectParentRepoMismatch(t *testing.T) {
	requireBinary(t, "jq")
...
func TestAssistantDXWorkspaceCreate_ProjectScopeRejectsFromWorkspace(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
func TestAssistantDXWorkspaceCreate_ProjectNotRegisteredProvidesDXRecovery(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
`)
	...
	if !strings.Contains(suggested, "project add --path /tmp/api") {
		t.Fatalf("suggested_command = %q, want assistant-dx project add command", suggested)
	}
	quickActions, ok := payload["quick_actions"].([]any)
	if !ok || len(quickActions) == 0 {
		t.Fatalf("quick_actions missing or empty: %#v", payload["quick_actions"])
