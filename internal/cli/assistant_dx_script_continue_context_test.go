...
func TestAssistantDXContinue_ContextWorkspaceWithoutAgentSurfacesOtherActiveAgents(t *testing.T) {
	requireBinary(t, "jq")
...
func TestAssistantDXContinue_ExplicitWorkspaceDoesNotRedirectToOtherAgents(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
func TestAssistantDXContinue_ContextWorkspaceQueryFailureSuggestsStatusAndAutoStart(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
`)
	...
	suggested, _ := payload["suggested_command"].(string)
	if !strings.Contains(suggested, "assistant-dx.sh status") {
	...
	for _, raw := range quickActions {
		...
		id, _ := action["id"].(string)
		cmd, _ := action["command"].(string)
		if id == "status" && strings.Contains(cmd, "assistant-dx.sh status") {
			sawStatus = true
		}
...
func TestAssistantDXContinue_ContextWorkspaceOtherAgentsHeaderShowsTruncation(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
func TestAssistantDXContinue_NoTargetPrioritizesContextProjectAgents(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
func TestAssistantDXContinue_NoTargetMultipleAgentsShowsTruncationHeader(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
