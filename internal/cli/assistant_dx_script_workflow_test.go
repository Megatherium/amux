...
func TestAssistantDXWorkflowKickoff_RegistersProjectCreatesWorkspaceAndStartsTurn(t *testing.T) {
	requireBinary(t, "jq")
...
func TestAssistantDXWorkflowKickoff_IgnoresPresentScriptAndPrintsJSON(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
`)
	...
	env = withEnv(env, "AMUX_ASSISTANT_DX_TURN_SCRIPT", fakeTurnPath)
	env = withEnv(env, "AMUX_ASSISTANT_DX_SELF_SCRIPT", scriptPath)
	env = withEnv(env, "AMUX_ASSISTANT_PRESENT_SCRIPT", fakePresentPath)
...
func TestAssistantDXWorkflowDual_RunsImplementationThenReview(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
func TestAssistantDXWorkflowDual_NeedsInputAutoFallbackRunsReview(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
func TestAssistantDXWorkflowDual_ReviewNeedsInputUsesNeedsInputHeader(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
printf '%s' '{"ok":true,"mode":"run","status":"needs_input","overall_status":"needs_input","summary":"Need reviewer confirmation.","agent_id":"agent-review","workspace_id":"ws-1","assistant":"codex","next_action":"Reply to review prompt first.","suggested_command":"skills/amux/scripts/assistant-dx.sh continue --agent agent-review --text \"Continue\" --enter","quick_actions":[],"channel":{"message":"review needs input","chunks":["review needs input"],"chunks_meta":[{"index":1,"total":1,"text":"review needs input"}],"inline_buttons":[]}}'
`)
