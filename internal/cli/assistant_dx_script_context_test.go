...
func TestAssistantDXProjectPick_NameSupportsDisambiguation(t *testing.T) {
	requireBinary(t, "jq")
...
func TestAssistantDXGuide_RecommendsReplyWhenAgentNeedsInput(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
...
func TestAssistantDXStart_UsesSiblingTurnScriptWhenInvokedOutsideRepoRoot(t *testing.T) {
	...
	sourceScriptPath := filepath.Join("..", "..", "skills", "amux", "scripts", "assistant-dx.sh")
	...
	scriptDir := t.TempDir()
	runDir := t.TempDir()
	copiedScriptPath := filepath.Join(scriptDir, "assistant-dx.sh")
...
func TestAssistantDXStart_UsesContextWorkspaceAndAssistant(t *testing.T) {
	requireBinary(t, "jq")
	requireBinary(t, "bash")
