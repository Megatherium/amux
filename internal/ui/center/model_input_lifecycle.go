...
func (m *Model) updateLaunchAgent(msg messages.LaunchAgent) (*Model, tea.Cmd) {
	return m, m.createAgentTabWithMetadata(msg.Assistant, msg.Workspace, msg.TicketID, msg.TicketTitle, msg.Model, msg.AgentMode, msg.DraftTabID)
}
...
func (m *Model) updateTabSessionStatus(msg messages.TabSessionStatus) (*Model, tea.Cmd) {
	...
	tab.mu.Unlock()
	if agent != nil {
		m.agentManager.CloseAgent(agent)
	}
	tab.mu.Lock()
	tab.Running = false
	tab.Detached = false
