...
// Dialog/picker context sentinel for internal ticket loading.
type ticketsForPickerLoaded struct {
	tickets []tickets.Ticket
}

// ---- Dialog show handlers ----
...
func (u *UICompositor) ShowSelectAssistantDialog(activeWorkspace *data.Workspace) {
	...
	u.dialog = common.NewAgentPicker(u.config.AssistantNames())
	u.dialog.SetSize(u.width, u.height)
	u.dialog.SetShowKeymapHints(u.config.UI.ShowKeymapHints)
	u.dialog.Show()
}

// ShowSelectTicketDialog returns a command to load tickets and show the ticket picker dialog.
// If no ticket service is available for the active project, it falls back
// directly to the assistant picker.
func (u *UICompositor) ShowSelectTicketDialog(activeWorkspace *data.Workspace, activeProject *data.Project, svc *tickets.TicketService) tea.Cmd {
	...
	return func() tea.Msg {
		t, _ := loadOpenAndInProgress(context.TODO(), svc, activeProject.Path, 50)
		return ticketsForPickerLoaded{tickets: t}
	}
}

// HandleTicketsForPickerLoaded creates and shows the ticket picker with hierarchically sorted tickets.
...
func (u *UICompositor) ShowSettingsDialog(updateAvailable *update.CheckResult, updateServiceIsHomebrew bool, version string) {
...
// PersistSettingsThemeIfDirty saves the theme if it has been changed.
...
// ---- Input routing handlers ----

// HandleDialogInput routes input to the active dialog overlay.
...
// HandleFilePickerInput routes input to the active file picker overlay.
...
// HandleSettingsDialogInput routes input to the active settings dialog overlay.
